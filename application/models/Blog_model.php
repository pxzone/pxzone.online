...
class Blog_model extends CI_Model {
    ...
    public function generateBlogURLExtension ($length = 5) {
        $characters = '0123456789abcdef';
        $charactersLength = strlen($characters);
        $temp_id = '';
        for ($i = 0; $i < $length; $i++) {
   ...
   public function getArticleData($url){
    ...
    public function getArticleDataJS(){
        ...
        $query = $this->db->SELECT('article_id, article_pub_id, title, url, at.status, content, at.created_at, description, lead, article_image, category, ut.name as author, ut.profile_image')
            ->FROM('article_tbl as at')
            ->WHERE('url', $url)
            ->JOIN('article_category_tbl as act','act.id=at.category_id')
            ->JOIN('users_tbl as ut', 'ut.user_id=at.author','left')
            ->GET()->row_array();
        $min_read = round(str_word_count($query['content']) / 190, 0);
        ...
        $dataArr = array(
            ...
            'title'=>$query['title'],
            'description'=>$query['description'],
            'author'=>($query['author'] == 'Admin') ? 'PX Staff' : $query['author'],
            'author_type'=>($query['author'] == 'Admin') ? 'Content Contributor' : 'Content Contributor',
    ...
    public function getArticleDataID($id){
        $query = $this->db->SELECT('article_id, article_pub_id, at.created_at, title, category, description, article_image, url, content, status, lead, act.id as category_id')
            ->FROM('article_tbl as at')
    ...
    public function getArticlesHomePage($row_per_page, $row_no){
        $query = $this->db->SELECT('at.title, at.url, at.description, at.article_image, at.created_at, at.content')
            ->FROM('article_tbl as at')
            ->LIMIT($row_per_page, $row_no)
            ->WHERE('at.status','published')
            ...
            foreach($query as $q){
                ...
                $array = array(
                    ...
                    'url'=>base_url('article/').$q['url'],
                    'article_image'=>base_url().$q['article_image'],
                    'created_at'=>date('j M y', strtotime($q['created_at'])),
                );
                array_push($result, $array);
            }
        return $result;
    }
    public function getArticlesHomePageCount(){
        return$this->db->FROM('article_tbl as at')
            ->WHERE('at.status','published')
            ->GET()->num_rows();
    }
