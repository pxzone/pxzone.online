...
class Archive_model extends CI_Model {
    ...
    public function searchPostsContent($row_per_page, $row_no){
        ...
        foreach($posts as $post){
            $row_array = array(
                ...
                'title'=>$post['title'],
                'post'=>$post['post'],
                'date_posted'=>date('m d, Y H:i:s', strtotime($post['date_posted'])),
            );
    ...
    function highestKarmaToday($row_per_page, $row_no){
        $search = $this->input->get('keyword');
        $date_range = array('aklt.created_at >'=>date('Y-m-d 00:00:00', strtotime('-7 hours')), 'aklt.created_at <'=> date('Y-m-d H:i:s', strtotime('-7 hours')));
    ...
    function highestKarmaThisMonth($row_per_page, $row_no){
        $search = $this->input->get('keyword');
        $date_range = array('aklt.created_at >'=>date('Y-m-01 00:00:00', strtotime('-7 hours')), 'aklt.created_at <'=> date('Y-m-t H:i:s', strtotime('-7 hours')));
        $data['logs'] = $this->db->SELECT('uid, aklt.username, aut.position, SUM(karma_point) as karma, aut.karma as total_karma, aklt.created_at')
            ->WHERE("(aklt.username LIKE '%".$search."%')", NULL, FALSE)
            ->WHERE($date_range)
            ->GROUP_BY('aklt.username')
