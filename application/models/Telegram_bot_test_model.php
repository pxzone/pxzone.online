...
class Telegram_bot_test_model extends CI_Model {
    ...
    public function insertNewTrackURL($chat_id, $topic_url, $topic_title){
        $long_url = explode(".msg", $topic_url);
        ...
        $tg_data = array(
         ...
         $this->db->INSERT('track_topics_tbl', $tg_data);  
     }
    ...
    public function getTrackTopicsData($chat_id){
        return $this->db->SELECT('chat_id, title')
           ->ORDER_BY('id','desc')
           ->WHERE('chat_id', $chat_id)
           ->GET('track_topics_tbl')->result_array();
    }
    public function deleteTrackTopic($chat_id, $topic_id){
       return $this->db->WHERE('chat_id', $chat_id)
          ->WHERE('topic_id', $topic_id)
          ->DELETE('track_topics_tbl');
   }
