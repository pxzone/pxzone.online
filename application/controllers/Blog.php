...
class Blog extends CI_Controller {
    ...
    public function getArticleDataJS(){ 
        ...
        if (empty($article_data['title'])) {
            ...
            foreach ($article_tags as $t) {
                $hashtags = str_replace(' ','',ucwords($t['tags'])).',';
            }
            ...
            $data['result'] = '
                    ...
                    <div class="row mt-2">
                        <div class="col-lg-6 col-12 font-15 text-start">'.$article_data['created_at'].' &nbsp;•&nbsp; '.$article_data['min_read'].' min read</div>
                     </div>
                    ...
                    <div class="mt-3 mb-2 author-details text-left d-flex flex-row">
                        <div class="p-2">
                            <img src="'.base_url().$article_data['profile_image'].'" class="author-image" alt="author profile img" > 
                        </div>
                        <div class="p-2 author-details2">
                            <div class="author-name">'.$article_data['author'].'</div>
                            <div class="c-light-gray author-type">'.$article_data['author_type'].'</div>
                        </div>
                    </div>
            ...
            $data['result'] = '
                    ...
                    <div class="row mt-2">
                        <div class="col-lg-6 col-12 col-md-12 font-13 text-start">'.$article_data['created_at'].' • '.$article_data['min_read'].' min read</div>
                     </div>
    ...
    public function getArticlesHomePageJS(){
        $row_no = $this->input->get('page_no');
        // Row per page
        $row_per_page = 15;
        ...
        // All records count
        $all_count = $this->Blog_model->getArticlesHomePageCount();

        // Get records
        $result = $this->Blog_model->getArticlesHomePage($row_per_page, $row_no);

        // Pagination Configuration
        $config['base_url'] = base_url('api/v1/article/_get');
        $config['use_page_numbers'] = TRUE;
        $config['total_rows'] = $all_count;
        $config['per_page'] = $row_per_page;
