...
class Telegram_bot_test extends CI_Controller {
    ...
    public function callback() {
        ...
        if(isset($update['message'])){
    	    ...
    	    $telegram_data = $this->Telegram_bot_test_model->getTelegramData($chat_id);
            $ask_notify = "no";
            ...
            elseif($telegram_data['status'] == 'inactive' && $message_text !== '/start'){
    	        ...
    	        $this->Telegram_bot_test_model->insertNewPhrase($chat_id, $phrase);
                $this->sendMessage($post_data);

                $this->sendTrackPhrase($chat_id);
            }
    	        ...
    	        $this->Telegram_bot_test_model->insertNewTrackedUser($chat_id, $user);
                $this->sendMessage($post_data);

                $this->sendTrackUsers($chat_id);
            }
    	        ...
    	        $this->Telegram_bot_test_model->insertNewIgnoredUser($chat_id, $user);
                $this->sendMessage($post_data);

                $this->sendIgnoredUsers($chat_id);
            }
            else {
                $response_text = "Sorry! I can't understand";
                $post_data = array(
                    'chat_id' => $chat_id,
                    'text' => $response_text,
                );
        ...
        else if (isset($update['callback_query'])) 
        {
            ...
            if ($callback_data === 'yes_notify_btn') {
                ...
                $this->Telegram_bot_test_model->registerTelegramData($chat_id);
                $post_data = array(
                    'message_id' => $message_id,
                    'chat_id' => $chat_id,
                    'text' => $response_text,
            ...
            elseif ($callback_data === 'no_notify_btn') {
            ...
            elseif (strpos($callback_data, 'yes_username_btn') !== false) {
                $altt_username = substr($callback_data, 17);
                $this->Telegram_bot_test_model->updateTelegramDataStatus($chat_id, $altt_username);
            ...
            elseif ($callback_data === 'no_username_btn') {
            ...
            elseif ($callback_data === 'stop_notify_btn') {
                $this->Telegram_bot_test_model->deleteTelegramData($chat_id);
            ...
            elseif ($callback_data === 'track_phrase' ) {
               $this->editTrackPhrase($chat_id, $message_id);
            } 
            
            elseif ($callback_data === 'add_new_phrase') {
            ...
            elseif ($callback_data === 'add_new_users') {
            ...
            elseif ($callback_data === 'add_ignore_users') {
            ...
            elseif ($callback_data === 'track_users') {
                $this->editTrackUsers($chat_id, $message_id);
            } 
            elseif ($callback_data === 'ignore_user') {
                $this->editIgnoreUsers($chat_id, $message_id);
            } 
            elseif ($callback_data === 'go_back') {
                $response_text = "Hello $name, \n\nCheck what you can do.";
                $keyboard = array(
                    'inline_keyboard' => array(
                        array(
                ...
                $chat_id = $str_parts[1];
                $phrase = $str_parts[2];
                $this->Telegram_bot_test_model->deleteTrackedPhrase($chat_id, $phrase);
                $this->editTrackPhrase($chat_id, $message_id);
            }
                ...
                $chat_id = $str_parts[1];
                $username = $str_parts[2];
                $this->Telegram_bot_test_model->deleteTrackedUsername($chat_id, $username);
                $this->editTrackUsers($chat_id, $message_id);
            } 
                ...
                $chat_id = $str_parts[1];
                $username = $str_parts[2];
                $this->Telegram_bot_test_model->deleteIgnoredUsername($chat_id, $username);
                $this->editIgnoreUsers($chat_id, $message_id);
            } 
            elseif ($callback_data === 'go_back_to_phrases') {
                $this->editTrackPhrase($chat_id, $message_id);
            }
            elseif ($callback_data === 'go_back_to_users') {
                $this->editTrackUsers($chat_id, $message_id);
            }
            elseif ($callback_data === 'go_back_to_ignore_users') {
                $this->editIgnoreUsers($chat_id, $message_id);
            }
            else {
    ...
    public function forwardMessage($post_data){
        $telegram_api = $this->telegram_api->authKeys();
        $bot_token = $telegram_api['api_key_test'];
        $api_endpoint = "https://api.telegram.org/bot$bot_token/forwardMessage";
    ...
    public function sendTrackPhrase($chat_id){
        $phrase_data = $this->Telegram_bot_test_model->getTrackPhraseData($chat_id);
        $phrases = array();
    ...
    public function editTrackPhrase($chat_id, $message_id){
        $phrase_data = $this->Telegram_bot_test_model->getTrackPhraseData($chat_id);
        $phrases = array();
        foreach ($phrase_data as $pd) {      
        $row_array = array(
    ...
    public function sendTrackUsers($chat_id){

        $user_data = $this->Telegram_bot_test_model->getTrackUsersData($chat_id);
        $users = array();
    ...
    public function editTrackUsers($chat_id, $message_id){
        $user_data = $this->Telegram_bot_test_model->getTrackUsersData($chat_id);
        $users = array();
        foreach ($user_data as $ud) {      
        $row_array = array(
    ...
    public function editIgnoreUsers($chat_id, $message_id){
        $user_data = $this->Telegram_bot_test_model->getIgnoredUsersData($chat_id);
        $users = array();
        foreach ($user_data as $ud) {      
        $row_array = array(
    ...
    public function sendIgnoredUsers($chat_id){
    ...
    public function showTestData(){
        ...
        print_r($ignore_user_arr);

        // $str_explode = explode("", $ignore_user_arr);
        // $str_implode = implode(",", $ignored_users['username']);
        // $this->output->set_content_type('application/json')->set_output(json_encode($ignore_user_arr));
    }
