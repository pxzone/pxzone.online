...
class Archive_model extends CI_Model {
    ...
    public function getKarmaLogDataSort($row_per_page, $row_no){
        ...
        if($sort == 'most_karma_today'){
            $date_range = array('aklt.created_at >'=>date('Y-m-d 00:00:00'), 'aklt.created_at <'=> date('Y-m-d H:i:s'));
            $logs = $this->db->SELECT('uid, aklt.username, SUM(karma_point) as karma, total_karma, aklt.created_at')
                ->WHERE("(aklt.username LIKE '%".$search."%')", NULL, FALSE)
                ->WHERE($date_range)
                ->GROUP_BY('uid')
                ->LIMIT($row_per_page, $row_no)
